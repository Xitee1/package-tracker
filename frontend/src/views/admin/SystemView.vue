<template>
  <div class="p-6 max-w-5xl mx-auto">
    ...
    <div v-if="error" class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 px-4 py-3 rounded-md text-sm mb-6">
      {{ error }}
    </div>

    <div v-if="loading && !status" class="text-center py-12 text-gray-500 dark:text-gray-400">Loading system status...</div>
    ...
    <template v-if="status">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-5">
          ...
          <p class="text-2xl font-bold mt-1" :class="errorCount > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-gray-500'">
            {{ errorCount }}
          </p>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div v-if="workers.length === 0" class="p-8 text-center text-gray-500 dark:text-gray-400 text-sm">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="worker in workers" :key="worker.folder_id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                <td class="px-5 py-3 text-sm font-medium text-gray-900 dark:text-white">{{ worker.folder_id }}</td>
                <td class="px-5 py-3">
                  <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium"
                    :class="worker.running ? 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'"
                  >
                    <span
                ...
                <td class="px-5 py-3">
                  <span v-if="!worker.error" class="text-sm text-gray-400 dark:text-gray-500">None</span>
                  <span v-else class="text-sm text-red-600 dark:text-red-400">{{ worker.error }}</span>
                </td>
              </tr>
            </tbody>
          </table>
